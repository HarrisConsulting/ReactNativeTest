...
This document chronicles the complete journey from a broken CI/CD pipeline to a fully functional, production-ready GitHub Actions workflow for a React Native project. It serves as a troubleshooting guide and reference for future React Native CI/CD implementations.

**Final Result**: 5-job CI/CD pipeline with 100% success rate, ~1m18s execution time.
...
**💥 Impact:** Pipeline failed immediately (0s elapsed) with YAML validation errors.
...
**💥 Impact:** `npm ci` failed because Ubuntu runners don't have CocoaPods installed.
...
**💥 Impact:** Unit tests failed because Jest couldn't parse ES modules from React Navigation.
...
**💥 Impact:** Lint job failed because ESLint didn't recognize `jest` as a global variable.
...
*This guide represents the complete troubleshooting journey from a non-functional to a fully operational CI/CD pipeline for React Native development.*
