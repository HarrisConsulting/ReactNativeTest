...
if [ -n "$TEAM_ID" ] && [ "$TEAM_ID" != '""' ]; then
    ...
    echo ""
    echo -e "${BLUE}💡 To build (Simulator - Recommended):${NC}"
    echo -e "${BLUE}   npx react-native run-ios --simulator=\"iPhone 16 Plus\"${NC}"
    echo -e "${BLUE}   # or iPhone 15:${NC}"
    echo -e "${BLUE}   npx react-native run-ios --simulator=\"iPhone 15\"${NC}"
    echo ""
    echo -e "${BLUE}💡 To build for Physical Device:${NC}"
    echo -e "${BLUE}   npm run ios${NC}"
else
    echo -e "${YELLOW}Team ID: Not set${NC}"
    echo -e "${GREEN}✅ Ready for Simulator build!${NC}"
    echo ""
    echo -e "${BLUE}💡 To build (Simulator - No Team ID Required):${NC}"
    echo -e "${BLUE}   npx react-native run-ios --simulator=\"iPhone 16 Plus\"${NC}"
    echo -e "${BLUE}   # or iPhone 15:${NC}"
    echo -e "${BLUE}   npx react-native run-ios --simulator=\"iPhone 15\"${NC}"
    echo ""
    echo -e "${YELLOW}💡 For Physical Device (Requires Team ID):${NC}"
    echo -e "${YELLOW}   1. Open: $WORKSPACE_FILE${NC}"
    echo -e "${YELLOW}   2. Select project → Target → Signing & Capabilities${NC}"
    echo -e "${YELLOW}   3. Set Team and verify Bundle Identifier${NC}"
    echo -e "${YELLOW}   4. Then run: npm run ios${NC}"
fi
